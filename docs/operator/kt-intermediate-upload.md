...
3. Review preview details (window, totals context, and row count).
4. If prompted, approve missing player creation (`POST /players`).
5. Approve final apply confirmation (`POST /apply`).
6. Validate `Apply result` output in terminal and keep it with operator logs.
